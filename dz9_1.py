"""Напишите следующие функции:
* Нахождение корней квадратного уравнения
* Генерация csv файла с тремя случайными числами в каждой строке. 100-1000 строк.
* Декоратор, запускающий функцию нахождения корней квадратного уравнения с каждой тройкой чисел из csv файла.
* Декоратор, сохраняющий переданные параметры и результаты работы функции в json файл."""

import csv
import json
import random
from cmath import sqrt
from pathlib import Path
from typing import Callable
from functools import wraps


def rand_csv(file_name: str):
    def deco(func: Callable):
        @wraps(func)
        def wrapper(**kwargs):
            with open(file_name, 'r', encoding='utf-8') as csv_file:
                reader = csv.reader(csv_file)
                for i, row in enumerate(reader):
                    if i == 0:
                        continue
                    args = (complex(j) for j in row)
                    yield func(*args, **kwargs)

        return wrapper

    return deco


def save_to_json(func):
    json_file_path = Path(f"{func.__name__}.json")
    if json_file_path.is_file():
        with open(json_file_path, 'r', encoding='utf-8') as f:
            json_file = json.load(f)
    else:
        json_file = []

    def wrapper(*args, **kwargs):
        for result in func(*args, **kwargs):
            if result:
                dct = {'args': args, **kwargs, 'result': str(result)}
                json_file.append(dct)
                with open(json_file_path, 'w', encoding='utf-8') as json_f:
                    json.dump(json_file, json_f, indent=2)
            else:
                break

    return wrapper


@save_to_json
@rand_csv('random.csv')
def quadrat(a: complex, b: complex, c: complex):
    if a != 0:
        discr: complex = b * b - 4 * a * c
        x1: complex = (-b + sqrt(discr)) / (2 * a)
        x2: complex = (-b - sqrt(discr)) / (2 * a)
        return discr, x1, x2
    else:
        return 0, 0, 0


def gen_csv_with_nums(name: str = 'random', rows_count: int = 500, min_num: int = -1000, max_num: int = 1000):
    rows = [{'a': random.randint(min_num, max_num), 'b': random.randint(min_num, max_num), 'c':
        random.randint(min_num, max_num)} for _ in range(rows_count)]
    with open(name + '.csv', 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['a', 'b', 'c']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)


if __name__ == '__main__':
    gen_csv_with_nums()
    quadrat()
